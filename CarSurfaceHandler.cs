...
public class CarSurfaceHandler : MonoBehaviour
{
    ...
    Surface.SurfaceTypes driveingOnSurface = Surface.SurfaceTypes.Road;
    private void Awake()
    {
    ...
    void Update()
    {
        ...
        for (int i = 0; i < numberOfHits; i++)
        {
            ...
            {
                driveingOnSurface = surface.surfaceTypes;
                lastSurfaceZValue = surface.transform.position.z;
            }
        }
        if (numberOfHits == 0)
            driveingOnSurface = Surface.SurfaceTypes.Road;
        lastSampledSurfacePosition
             = transform.position;
        Debug.Log($"Driving on {driveingOnSurface}");
    }
   public Surface.SurfaceTypes GetCurrrentSurface()
    {
        return driveingOnSurface;
    }
}
